...
The `make_tiles()` function currently supports 'square', 'hexagon', 'circle', and 'diamond' tile shapes:
...
ggplot2::ggplot() +
  ggplot2::geom_polygon(
    ggplot2::aes(x, y, group = id), full_data
  ) + 
  ggplot2::coord_equal() +
...
We can also join (merge) the tile map points data with our own data sets. This can be useful for better labeling or, in the case below, we've added a gradient fill to our tiles, scaling with a joined variable *sales*.
...
This package contains some ready-made matrix layouts. From popular country's states to European nations, the matrix can be loaded in short order using `tile_layouts()`:
...
plot_tiles(euro_data) + ggplot2::coord_equal()
```
