...
#' @param size An integer detailing the point radius of the hexagon tile shape.
#' @param padding An integer detailing the space between surrounding hexagon
#' tile shapes.
#' @param start A vector of length two comprised of the x,y position for the next
#' hexagon tile shape to draw.
#' @seealso [make_hex_tiles(), make_tiles()]
...
#' Make Hexagon Tile Data
#'
#' `make_hex_tiles()` creates a data frame of tile data points for each
...
make_hex_tiles <- function(layout, size, padding) {
  ...
  for(i in row_order) {
    for(j in col_order) {
      ...
      # Draw the shape
      data_gen = gen_hex(group_id, size, padding, pos)
